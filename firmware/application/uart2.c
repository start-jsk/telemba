/*
  ...
  ********************************************************************************
  ...
  without Microchip痴 prior written consent in each instance.
...
static char uart2_recv_buff[256];
static int uart2_recv_head;
static int uart2_recv_tail;
...
{
    /* 1byte受信 */
    uart2_recv_buff[uart2_recv_tail] = U2RXREG;
    uart2_recv_tail = (uart2_recv_tail + 1) % sizeof(uart2_recv_buff);
    IFS1bits.U2RXIF = 0;
}
 ...
 * バッファの受信サイズを返す
 */
int uart2_recv_get_size (void)
{
    return (uart2_recv_tail - uart2_recv_head) % sizeof(uart2_recv_buff);
}

/**
 * バッファからデータを取得
 */
int uart2_recv_get_data (char *data, int size)
{
    int i;
    if (size > sizeof(uart2_recv_buff)) {
	size = sizeof(uart2_recv_buff);
    }
    /* 受信割り込みの禁止 */
    IEC1bits.U2RXIE=0;
    if (uart2_recv_get_size() < size) {
	size = uart2_recv_get_size();
    }
    for (i=0; i<size; i++) {
	data[i] = uart2_recv_buff[uart2_recv_head];
	uart2_recv_head = (uart2_recv_head + 1) % sizeof(uart2_recv_buff);
    }
    /* 受信割り込みの許可 */
    IEC1bits.U2RXIE=1;
    return size;
 ...
 * バッファからデータを取得
 */
char uart2_recv_get_char (void)
{
    char c;
    /* 受信割り込みの禁止 */
    IEC1bits.U2RXIE=0;
    if (uart2_recv_get_size() == 0) {
	/* 受信割り込みの許可 */
	IEC1bits.U2RXIE=1;
	return -1;
    }
    c = uart2_recv_buff[uart2_recv_head];
    uart2_recv_head = (uart2_recv_head + 1) % sizeof(uart2_recv_buff);
    /* 受信割り込みの許可 */
    IEC1bits.U2RXIE=1;
    return c;
 ...
 * 送信バッファ関連
 */
static unsigned char uart2_send_buff[256];
static int uart2_send_head;
static int uart2_send_tail;
...
{
    /* FIFOが空の場合 */
    if (uart2_send_head == uart2_send_tail) {
        IFS1bits.U2TXIF = 0;
        return;
    }
    /* 1byte送信 */
    U2TXREG = uart2_send_buff[uart2_send_tail];
    uart2_send_tail = (uart2_send_tail + 1) % sizeof(uart2_send_buff);
    IFS1bits.U2TXIF = 0;
}
 ...
 * 送信バッファにデータを追加
 */
int uart2_send_data (unsigned char *data, int size)
{
    int i;
    if (size > sizeof(uart2_send_buff)) {
	size = sizeof(uart2_send_buff);
    }
    /* 送信割り込みの禁止 */
    IEC1bits.U2TXIE=0;
    for (i=0; i<size; i++) {
	uart2_send_buff[uart2_send_head] = data[i];
	uart2_send_head = (uart2_send_head + 1) % sizeof(uart2_send_buff);
    }
    /* 送信割り込みの許可 */
    IEC1bits.U2TXIE=1;
    return size;
